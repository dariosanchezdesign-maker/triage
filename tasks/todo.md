...
## Found the real cause: max_tokens was too small, not a timeout (2026-08-16)

- Added diagnostic logging (elapsed ms, exit reason, Anthropic's `stop_reason`, whether `message_stop` was seen) to the stream handler since the previous fix (trimming max_tokens to 2200) still produced generic client errors, and the silent error-swallowing made it impossible to tell why.
- Live log after redeploy: `stream ended after 26943ms, exit=natural-done, stopReason=max_tokens, messageStopSeen=...` -- confirmed definitively via Anthropic's own stop_reason field, not inferred: the function wasn't timing out or erroring at all. `max_tokens: 2200` was simply too small for a real 10-stage breakdown even with the tightened reasoning-length prompt instructions from the previous pass -- Anthropic was cutting the response off mid-generation, producing syntactically incomplete JSON that failed to parse client-side (silently, since the HTTP status was a clean 200).
- Computed a better number from real data instead of guessing again: 2200 tokens took ~27s to generate -> ~81 tokens/sec. Set `max_tokens: 3200`, which computes to ~40s at that rate -- comfortable margin under the 60s streaming-function ceiling, while actually large enough to let the model finish.
- Also worth noting for later: one test via direct `fetch()` returned in 4ms with just the sentinel and no content, inconsistent with the 27s+ durations seen everywhere else -- not yet explained, possibly a fluke in that one measurement. Flagging rather than ignoring, but not chasing further right now since the max_tokens fix addresses the reproducible, log-confirmed failure mode.
- Not yet re-tested live with the new max_tokens value -- next step is to push and re-run the same full-breakdown flow.
